...
### UI 개선
- 🎨 **이미지 라이트박스 개선**: 전체 화면 → 배너 형태로 변경
  - 스타일 이미지 미리보기를 화면 중앙 배너로 표시
  - 배경 오버레이 클릭으로 닫기 기능 개선
  - 부드러운 슬라이드 인 애니메이션 추가
...
- ✅ **Animagine XL 자동 전환**: 애니메이션 스타일 선택 시 전용 모델로 자동 전환
...
2. **모델 다운로드** (필수)
   - **Juggernaut XL v9**: 기본 모델 (실사/웹툰용)
   - **Animagine XL v3.1**: 애니메이션 전용 모델 (선택)
