 ...
 * @param {string} apiKey - API 키 (암호화되어 저장됨)
...
export async function saveUserApiSettings(username, apiType, apiKey, projectId = null) {
    // apiType 유효성 검사
    const validApiType = (apiType === 'vertex_ai') ? 'vertex_ai' : 'ai_studio';
    ...
    console.log(`✅ API 설정 저장됨: ${username} (${validApiType})`);
}
...
export async function getUserApiSettings(username) {
    ...
    if (!user) {
        return { apiType: 'ai_studio', apiKey: null, projectId: null };
    }

    const apiKey = user.apiKeyEncrypted ? decrypt(user.apiKeyEncrypted) : null;

    // apiType이 없거나 유효하지 않으면 기본값 'ai_studio'
    const apiType = (user.apiType === 'vertex_ai') ? 'vertex_ai' : 'ai_studio';
